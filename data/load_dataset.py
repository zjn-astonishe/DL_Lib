...
def read_train_valid_data(root:str):
    random.seed(0)  # 保证随机结果可复现
    assert os.path.exists(root), "dataset root: {} does not exist.".format(root)
    ...
    for cla in dataset_class:
        ...
        vallid_cla_path = os.path.join(valid_root, cla)
        # 遍历获取supported支持的所有文件路径
        train_images = [os.path.join(train_root, cla, i) for i in os.listdir(train_cla_path)
                  if os.path.splitext(i)[-1] in supported]
        valid_images = [os.path.join(valid_root, cla, i) for i in os.listdir(vallid_cla_path)
                  if os.path.splitext(i)[-1] in supported]
        # 排序，保证各平台顺序一致
        train_images.sort()
        valid_images.sort()
        ...
        image_class = class_indices[cla]
        # 记录该类别的样本数量
        every_class_num.append(len(train_images+valid_images))
